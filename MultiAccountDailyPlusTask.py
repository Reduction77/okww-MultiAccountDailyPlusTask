...
v1.4 修复切号中途重新抢前台导致点不上/选号死循环：
- 问题：v1.3 只在各流程入口让出一次焦点，但游戏在每一次登录切换时都会重新
  抢前台，后续步骤（esc、确认弹窗、下拉框、选号、登录按钮）仍落在前台状态；
  且前台时点击会落到物理鼠标所在位置，选号一直点中同一个账号，陷入死循环。
- 修复：把让出焦点的检查下沉到 click/send_key 层——本任务的每一次脚本点击/
  按键之前都会先检查并让出前台，游戏任何时候抢前台都会被立即让回，
  覆盖整个任务全程（含官方选号流程内部的每一步），不再漏任何一次输入。
"""
...
class MultiAccountDailyPlusTask(MultiAccountDailyTask):
    ...
    def click(self, *args, **kwargs):
        ...
        游戏在每一次登录切换时都会重新抢前台（前台 RawInput 会无视后台点击，
        或让点击落到物理鼠标位置），只在流程入口让一次焦点会漏掉后续步骤。
        下沉到 click 层后，官方选号流程内部的每一步点击也都被覆盖。
        """
        self._yield_foreground_if_game_front()
        return super().click(*args, **kwargs)

    def send_key(self, *args, **kwargs):
        """脚本按键前必检：游戏在前台就先让出焦点（esc 等按键同样会被前台吞掉）。"""
        self._yield_foreground_if_game_front()
